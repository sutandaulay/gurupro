...
import { AttendanceLogSchema } from '@/lib/validations/attendance';
...
async function performAntiFraudChecks(
  validatedData: z.infer<typeof CheckOutSchema>,
  ipAddress: string,
  ...
  // 6. Speed sanity check - deteksi teleportation
  const speedResult = await checkImpossibleSpeed(validatedData.teacherId, validatedData.latitude, validatedData.longitude, new Date());
  if (speedResult.isImpossible) {
    trustScore -= 0.25;
    flagReasons.push('impossible_speed');
  }
...
// Fungsi helper untuk cross-check IP vs GPS
async function checkIPGPSMismatch(ipAddress: string, lat: number, lng: number): Promise<{ isMismatch: boolean }> {
  // Dalam implementasi nyata, ini akan panggil service geolocation berdasarkan IP
  // Menggunakan API seperti ip-api.com atau ipinfo.io
  try {
    // Skip untuk IP lokal atau localhost
    if (ipAddress === 'unknown' || ipAddress.startsWith('127.') || ipAddress.startsWith('192.168.') || ipAddress.startsWith('10.')) {
      return { isMismatch: false };
    }

    // Contoh implementasi dengan ip-api.com (gratis tier) - UNCOMMENT untuk produksi
    // const response = await fetch(`http://ip-api.com/json/${ipAddress}`);
    // const data = await response.json();
    // if (data.status === 'success') {
    //   const ipLat = data.lat;
    //   const ipLng = data.lon;
    //   const distance = calculateDistance(lat, lng, ipLat, ipLng);
    //   // Jika jarak > 100km, kemungkinan besar IP dan GPS tidak cocok
    //   return { isMismatch: distance > 100000 };
    // }

    return { isMismatch: false }; // Default untuk simulasi
  } catch (error) {
    console.error('IP-GPS check error:', error);
    return { isMismatch: false };
  }
}

// Fungsi helper untuk speed sanity check
async function checkImpossibleSpeed(
  teacherId: string,
  lat: number,
  lng: number,
  currentTime: Date
  ...
  try {
    // Ambil presensi terakhir dari guru ini (masuk atau pulang)
    ...
    if (lastLog.length === 0) {
      return { isImpossible: false }; // Tidak ada data sebelumnya
    }

    const prevLat = lastLog[0].latitude!;
    const prevLng = lastLog[0].longitude!;
    const prevTime = lastLog[0].timestamp;

    // Hitung jarak antara dua titik
    const distanceMeters = calculateDistance(prevLat, prevLng, lat, lng);
    const timeDiffMs = currentTime.getTime() - new Date(prevTime).getTime();
    ...
    // Batas kecepatan wajar: 120 km/jam (di jalan tol)
    const maxReasonableSpeed = 120;
    return {
      isImpossible: speedKmh > maxReasonableSpeed,
...
// Fungsi helper untuk menghitung jarak antar koordinat
function distanceBetweenCoordinates(lat1: number, lng1: number, lat2: number, lng2: number): number {
...
async function updateDailyAttendanceSummary(teacherId: string, institutionId: string) {
  ...
  if (existingSummary) {
    // Update summary yang ada dengan waktu check-out
    ...
    // Buat summary baru jika tidak ada check-in sebelumnya
