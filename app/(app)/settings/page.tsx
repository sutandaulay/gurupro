...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      fetchProfile().then(data => { if (data?.id) setUser(data); });
    }
  }, []);
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {!user ? (
          ...
          <div className="space-y-6">
            ...
            {/* Pemetaan Kolom Raport */}
            <div id="pemetaan-kolom">
              <PemetaanKolomSection />
            </div>
          </div>
        )}
      </div>
...
function PemetaanKolomSection() {
  ...
  if (!activeSchoolId && schools.length === 0) {
    return (
      <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
        <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Pemetaan Kolom Raport</h2>
        <p className="text-xs text-slate-400">
          Pilih sekolah terlebih dahulu di menu Dasbor untuk mengatur pemetaan kolom ekspor Excel raport.
        </p>
      </div>
    );
  }
