...
function AdministrasiContent() {
  ...
  const convertMarkdownToHtml = (md: string): string => {
    ...
    let html = md
      ...
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
      .replace(/!\[(.*?)\]\((.*?)\)/g, '<img src="$2" alt="$1" class="rounded-2xl max-w-full my-4 shadow-md border border-slate-200 mx-auto block" />')
      .replace(/^### (.+)$/gm, '<h3 class="text-base font-bold text-slate-800 mt-4 mb-2">$1</h3>')
      .replace(/^## (.+)$/gm, '<h2 class="text-lg font-bold text-slate-800 border-b border-slate-200 pb-1 mt-6 mb-3">$1</h2>')
      .replace(/^# (.+)$/gm, '<h1 class="text-xl font-bold text-slate-900 border-b-2 border-slate-300 pb-2 mt-8 mb-4 uppercase text-center">$1</h1>')
      .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
      .replace(/\*(.+?)\*/g, '<em>$1</em>')
      .replace(/^- (.+)$/gm, '<li class="list-disc ml-6 my-1">$1</li>')
      .replace(/^(\d+)\. (.+)$/gm, '<li class="list-decimal ml-6 my-1">$2</li>')
      .replace(/\n\n/g, "</p><p class='my-2 text-justify'>")
      .replace(/\n/g, "<br>");

    html = html.replace(/(<li class="list-disc ml-6 my-1">.*<\/li>\n?)+/g, '<ul class="my-3">$&</ul>');
    html = html.replace(/(<li class="list-decimal ml-6 my-1">.*<\/li>\n?)+/g, '<ol class="my-3">$&</ol>');

    // Handle markdown tables
    html = html.replace(/\|(.+)\|\n\|[-:\s|]+\|\n((?:\|.+\|\n?)+)/g, (match: string, header: string, body: string) => {
      const headerCells = header.split('|').filter((c: string) => c.trim()).map((c: string) => `<th class="border border-slate-300 px-3 py-2 bg-slate-100 font-semibold text-center">${c.trim()}</th>`).join('');
      const headerRow = `<tr>${headerCells}</tr>`;
      const bodyRows = body.trim().split('\n').map((row: string) => {
        const cells = row.split('|').filter((c: string) => c.trim() !== undefined).slice(1, -1).map((c: string) => `<td class="border border-slate-300 px-3 py-2">${c.trim()}</td>`).join('');
        return `<tr>${cells}</tr>`;
      }).join('');
      return `<table class="w-full border-collapse border border-slate-300 my-4 text-sm"><thead>${headerRow}</thead><tbody>${bodyRows}</tbody></table>`;
    });

    return `<div class="markdown-body text-sm text-slate-700 leading-relaxed font-sans">${html}</div>`;
  };
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 min-h-[600px] print:border-none print:shadow-none print:p-0">
              {!result ? (
                ...
                <>
                  ...
                  {/* Rendering Content */}
                  <div 
                    className="prose max-w-none print:max-w-none px-2"
                    dangerouslySetInnerHTML={{ __html: convertMarkdownToHtml(result.konten) }}
                  />
                </>
              )}
            </div>
