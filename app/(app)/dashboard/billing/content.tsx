...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      // Process referral code for Google OAuth users
      const refCode = params.get("ref");
      if (refCode) {
        apiFetch("/api/auth/referral/process", {
          method: "POST",
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex gap-6">
              <div className="text-center">
                ...
                <p className="text-xl font-semibold text-gray-900">{tokenStatus?.quota_poin_available ?? tokenStatus?.token_limit ?? 0}</p>
              </div>
              <div className="text-center">
                 <p className="text-xs text-gray-500 uppercase tracking-wide">Top-up</p>
                 <div className="mt-1">
                   <p className="text-xl font-semibold text-green-600">{tokenStatus?.addon_poin_available ?? tokenStatus?.addon_token_balance ?? 0}</p>
