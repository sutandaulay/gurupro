...
export default function OperatorPage({
  ...
  const institutionId = urlParams.institutionId as string;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="secondary" size="sm" onClick={() => setShowAcademicModal(true)}>
            Kelola
          </Button>
        </div>
      </div>

      {activeMembers.filter((m) => m.assigned_mapel.length === 0 && m.assigned_kelas.length === 0).length > 0 && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-amber-800">
                  {activeMembers.filter((m) => m.assigned_mapel.length === 0 && m.assigned_kelas.length === 0).length} guru belum ter-assign kelas/mapel
                </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: "Total Anggota", value: members.length, color: "text-gray-900" },
          { label: "Aktif", value: activeMembers.length, color: "text-green-600" },
          { label: "Menunggu Konfirmasi", value: invitedMembers.length, color: "text-amber-600" },
        ].map((s) => (
      ...
      <DapodikExportCard
        ...
        academicYear={institution?.academic_year_active || "2025/2026"}
      />
...
function MembersTable({
  ...
  const handleToggleStatus = async (member: Member) => {
    ...
    if (!window.confirm(`${newStatus === "left" ? "Nonaktifkan" : "Aktifkan"} anggota ${member.nama_lengkap || member.cms_user_name}?`)) return;
                ...
                <tr key={m.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      {m.status === "active" && (
                        <Button variant="ghost" size="sm" onClick={() => onResetPassword(m)}>
                          Reset PW
                        </Button>
                      )}
...
function AddTeacherModal({ institutionId, onClose, onSuccess }: { institutionId: number; onClose: () => void; onSuccess: () => void }) {
  const toast = useToast();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Tambah Guru (Manual)</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          <Input label="Nama Lengkap" required placeholder="Masukkan nama guru" value={form.nama} onChange={(e) => setForm({ ...form, nama: e.target.value })} error={errors.nama || null} />
          <Input label="Email" required type="email" placeholder="guru@sekolah.sch.id" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} error={errors.email || null} />
          <Input label="NIK (opsional)" placeholder="Masukkan NIK" value={form.nik} onChange={(e) => setForm({ ...form, nik: e.target.value })} error={errors.nik || null} />
          <Input label="Mata Pelajaran (opsional)" placeholder="Pisahkan dengan koma, misal: Matematika, IPA" value={form.mapel} onChange={(e) => setForm({ ...form, mapel: e.target.value })} />
          <Input label="Kelas (opsional)" placeholder="Pisahkan dengan koma, misal: 7A, 7B" value={form.kelas} onChange={(e) => setForm({ ...form, kelas: e.target.value })} />
...
function ImportExcelModal({ institutionId, onClose, onSuccess }: { institutionId: number; onClose: () => void; onSuccess: () => void }) {
  const toast = useToast();
  const [file, setFile] = useState<File | null>(null);
  const [importing, setImporting] = useState(false);
  const [result, setResult] = useState<{ total: number; berhasil: number; gagal: number; sudah_punya_akun_individual: number; details: { baris: number; email: string; status: string; keterangan: string }[] } | null>(null);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/institution/${institutionId}/members/import`, { method: "POST", body: formData });
      const data = await res.json();
      if (res.ok) { setResult(data); toast.success("Proses impor selesai"); }
      else { toast.error(data.error || "Gagal mengimpor data"); }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Import Guru dari Excel</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none cursor-pointer">&times;</button>
        </div>
          ...
          {!result ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="file" accept=".xlsx" onChange={(e) => setFile(e.target.files?.[0] || null)} className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-violet-50 file:text-violet-700 hover:file:bg-violet-100 cursor-pointer" />
              </div>
              <div className="flex justify-end gap-3 pt-2">
                <Button variant="secondary" onClick={onClose}>Batal</Button>
                <Button variant="primary" onClick={handleImport} loading={importing} disabled={!file}>Import</Button>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-green-50 rounded-lg p-3 text-center"><p className="text-2xl font-bold text-green-600">{result.berhasil}</p><p className="text-xs text-green-700">Berhasil</p></div>
                <div className="bg-red-50 rounded-lg p-3 text-center"><p className="text-2xl font-bold text-red-600">{result.gagal}</p><p className="text-xs text-red-700">Gagal</p></div>
                <div className="bg-amber-50 rounded-lg p-3 text-center"><p className="text-2xl font-bold text-amber-600">{result.sudah_punya_akun_individual}</p><p className="text-xs text-amber-700">Punya Akun Individual</p></div>
              </div>
              {result.details.length > 0 && (
                <div className="max-h-48 overflow-y-auto border rounded-lg">
                  <table className="w-full text-xs">
                    <thead><tr className="bg-gray-50 border-b"><th className="text-left px-2 py-1.5">Baris</th><th className="text-left px-2 py-1.5">Email</th><th className="text-left px-2 py-1.5">Status</th><th className="text-left px-2 py-1.5">Keterangan</th></tr></thead>
                    <tbody>
                      {result.details.map((d, i) => (
                        <tr key={i} className={d.status === "gagal" ? "bg-red-50" : "bg-green-50"}>
                          <td className="px-2 py-1.5">{d.baris}</td><td className="px-2 py-1.5">{d.email}</td>
                          <td className="px-2 py-1.5"><Badge variant={d.status === "gagal" ? "error" : d.status === "sudah_aktif" ? "success" : "info"}>{d.status === "gagal" ? "Gagal" : d.status === "sudah_aktif" ? "Sudah Aktif" : "Diundang"}</Badge></td>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button variant="primary" onClick={() => { setResult(null); setFile(null); onSuccess(); }}>Selesai</Button>
              </div>
            </div>
          )}
        </div>
...
function ResetPasswordModal({ member, institutionId, onClose }: { member: Member; institutionId: number; onClose: () => void }) {
  const toast = useToast();
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { toast.success("OTP reset password dikirim", data.message); onClose(); }
      else { toast.error(data.error || "Gagal mengirim OTP"); }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Reset Password Guru</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none cursor-pointer">&times;</button>
        </div>
        <div className="px-6 py-4 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="secondary" onClick={onClose}>Batal</Button>
            <Button variant="primary" onClick={handleReset} loading={sending}>Kirim OTP Reset</Button>
          </div>
        </div>
      </div>
    </div>
...
function EditAssignmentModal({ member, onClose, onSuccess }: { member: Member; onClose: () => void; onSuccess: () => void }) {
  const toast = useToast();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { toast.success("Assignment berhasil diperbarui"); onSuccess(); }
      else { toast.error(data.error || "Gagal memperbarui assignment"); }
    } catch { toast.error("Gagal memperbarui assignment"); }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Edit Assignment: {member.nama_lengkap || member.cms_user_name}</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none cursor-pointer">&times;</button>
        </div>
        <div className="px-6 py-4 space-y-4">
          <Input label="Mata Pelajaran" placeholder="Pisahkan dengan koma" value={mapel} onChange={(e) => setMapel(e.target.value)} />
          <Input label="Kelas" placeholder="Pisahkan dengan koma" value={kelas} onChange={(e) => setKelas(e.target.value)} />
          <div className="border-t border-gray-200 pt-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <select className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-indigo-400 focus:outline-none" value={subRole} onChange={(e) => setSubRole(e.target.value)}>
              ...
              {subRole === "wali_kelas" && (
                <Input label="Kelas Wali" placeholder="Contoh: VII-A, X-MIPA-1" value={waliKelasOf} onChange={(e) => setWaliKelasOf(e.target.value)} helperText="Nama kelas yang diasuh sebagai Wali Kelas" />
              )}
              {subRole === "pembina_ekskul" && (
                <Input label="Nama Ekskul" placeholder="Contoh: Pramuka, PMR, Basket" value={ekskulName} onChange={(e) => setEkskulName(e.target.value)} helperText="Nama ekstrakurikuler yang diasuh" />
              )}
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-2">
...
function AcademicYearModal({ institution, onClose, onSuccess }: { institution: Institution; onClose: () => void; onSuccess: () => void }) {
  const toast = useToast();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { toast.success("Tahun ajaran berhasil diperbarui"); onSuccess(); }
      else { toast.error(data.error || "Gagal memperbarui tahun ajaran"); }
    } catch { toast.error("Gagal memperbarui tahun ajaran"); }
    finally { setSaving(false); }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Kelola Tahun Ajaran</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none cursor-pointer">&times;</button>
        </div>
        <div className="px-6 py-4 space-y-4">
          <Input label="Tahun Ajaran Aktif" required placeholder="Contoh: 2025/2026" value={academicYear} onChange={(e) => setAcademicYear(e.target.value)} helperText="Ubah tahun ajaran aktif untuk institusi ini." />
...
function DapodikExportCard({ institutionId, academicYear }: { institutionId: number; academicYear: string }) {
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const qs = new URLSearchParams({ institutionId: String(institutionId), semester, tahunAjaran: academicYear, version });
      const res = await apiFetch(`/api/export/dapodik?${qs.toString()}`);
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
        window.alert(d.error || "Gagal mengekspor file");
        return;
      }
    ...
    } catch { window.alert("Terjadi kesalahan saat mengekspor"); }
    ...
    <Card className="p-5 mb-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-3">
        <div>
          ...
          <Select value={semester} options={[{ value: "ganjil", label: "Ganjil" }, { value: "genap", label: "Genap" }]} onChange={(e) => setSemester(e.target.value as "ganjil" | "genap")} />
        </div>
        <div>
          <Label>Versi Dapodik</Label>
          <Select value={version} options={[{ value: "2025", label: "2025" }, { value: "2024", label: "2024" }]} onChange={(e) => setVersion(e.target.value)} />
        </div>
      </div>
