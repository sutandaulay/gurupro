...
export default function LaporanHarianPage() {
  ...
  const buildUrl = useCallback(() => {
    ...
    if (selectedSekolahId) params.set('sekolah_id', selectedSekolahId)
    return `/api/laporan-harian?${params.toString()}`
  }, [filter, selectedDate, selectedSekolahId])
  ...
  async function loadSchools() {
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        setSchoolList(data)
        const saved = localStorage.getItem('sekolahId') || ''
        if (saved && data.some((s: SchoolItem) => s.id === saved)) {
          setSelectedSekolahId(saved)
        } else if (data.length > 0) {
          setSelectedSekolahId(data[0].id)
          localStorage.setItem('sekolahId', data[0].id)
        }
      }
    } catch (err) {
  ...
  const handleSchoolChange = (val: string) => {
    setSelectedSekolahId(val)
    localStorage.setItem('sekolahId', val)
  }
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Riwayat aktivitas mengajar harian Anda
        </p>
      </div>
      ...
      {schoolList.length > 0 && (
        <div className="mb-4">
          <select
            ...
            className="px-4 py-2.5 rounded-xl text-sm font-semibold bg-white border border-slate-200 text-slate-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
          >
            {schoolList.map((s) => (
