...
export default function LaporanKinerjaListPage() {
  ...
  async function fetchData() {
    setLoading(true)
    try {
      const skpParams = selectedSekolahId ? `?sekolah_id=${selectedSekolahId}` : ''
      const lapParams = selectedSekolahId ? `?sekolah_id=${selectedSekolahId}` : ''
  ...
  useEffect(() => {
    if (selectedSekolahId !== undefined) {
      fetchData()
    }
  }, [selectedSekolahId])
    ...
    <div className="container max-w-4xl mx-auto py-6 px-4">
      <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
        <div>
          ...
          {schoolList.length > 1 && (
            <div className="mt-2">
              <label className="text-xs text-muted-foreground mr-2">Sekolah:</label>
              <select
